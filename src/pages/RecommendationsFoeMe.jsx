...
const RecommendationsForMe = () => {
	...
	const fetchRecommendations = async () => {
		...
		try {
			const res = await fetch(
				`https://smart-pick-server-seven.vercel.app/recommendationsForUser?userEmail=${user.email}`,
			)
	...
	if (loading)
		return <p className='text-center py-10'>Loading recommendations...</p>

	if (!recommendations.length)
		return (
			<p className='text-center py-10 text-gray-500'>No recommendations yet.</p>
		)
