...
const QueryDetailsPage = () => {
  ...
  const fetchQuery = async () => {
    ...
    try {
      ...
      const res = await fetch(`https://smart-pick-server.vercel.app/queries/${id}`, {
  ...
  const fetchRecommendations = async () => {
    ...
    try {
      const idToken = user ? await auth.currentUser.getIdToken() : null;
      const res = await fetch(`https://smart-pick-server.vercel.app/recommendations?queryId=${id}`, {
        headers: idToken ? { Authorization: `Bearer ${idToken}` } : {},
      });
